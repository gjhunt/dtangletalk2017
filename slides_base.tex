...
\begin{frame}
  ...
  \begin{columns}
    ...
    \begin{column}{.75\textwidth}
      ...
      \item can lead to worse conditions like Lyme encephalopathy, insomnia, or depression
      \end{enumerate}
          
      \begin{center}{\bf \alert{Bouquet et al:} try to understand the immune progression of Lyme.}\end{center}
    \end{column}
  \end{columns}
\end{frame}
...
\begin{frame}
  \frametitle{Study WBCs to Understand Immune Response to Lyme}
             {\bf Bouquet et al:} collect gene expression measurements or ``profiles'' (GEPs) of white blood cells (WBCs) of 
             \begin{enumerate}
             \item \alert{28 Lyme patients}
             \item and \alert{13 healthy controls}.
             \end{enumerate}

             The analysis compares GEPs across groups.\\

             \vspace{.5cm}\uncover<2->{{\large \alert{WBCs encompass many types:} B, T, NK, monocytes, ...}\\\vspace{.25cm}
               {\bf Understanding these sub-types would be helpful:}
               \begin{enumerate}
               \item tracking subtype composition changes over disease course
               \item adjusting expression comparisons across groups
               \end{enumerate}
             }

             \uncover<3->{{\bf {\cbr Problem:}} estimate the cell-type proportions of the samples using the gene expression data.}
\end{frame}

\begin{frame}
  \frametitle{Gene Expression Data}
  ``Gene expression measurements'' = What genes the cells are using\\
  \uncover<1->{{\large {\bf Measure expression using mRNA:}\\ Gene Expressed $\rightarrow$ \alert{mRNA transcript} created}\\
  \begin{center}
    \includegraphics[scale=.35]{pictures/mrna}
  \end{center}}
  \uncover<2->{{\bf Measuring gene expression = Quantifying mRNA transcripts}\\}
  \uncover<3->{\begin{center}
    \vspace{-.9cm}\hspace*{-.75cm}\includegraphics[scale=.25]{pictures/measure_expr}
  \end{center}}
\end{frame}

\begin{frame}
  \frametitle{Estimating Cell-type Proportions}
  ...
  \hspace{1cm}\begin{center}
    ...
    \end{overprint}
  \end{center}
  \alert{\large Goal:} estimate cell-type proportions
...
\begin{frame}
  ...
  \begin{enumerate}
  \item Can be as simple as fitting using sub-matrices.
...
\begin{frame}
  \frametitle{dtangle in a Simple Setting}
...
\begin{frame}
  \frametitle{{\bf dtangle} is a New Cell-type Estimation Method}
 \vspace*{-.5cm}
  \begin{center}
    \begin{figure}
...
\begin{frame}
  \frametitle{Combining dtangle's Models}  
...
\begin{frame}
  \frametitle{dtangle: A Re-normalization of the Exponential Terms.}
...
\begin{frame}
  \frametitle{Marker Genes and $\gamma$}
  \begin{enumerate}
  ...
  \end{enumerate}
  {\bf dtangle is robust to changes in $\gamma$ and marker genes.}
\end{frame}
...
\begin{frame}
  \frametitle{dtangle is Robust}
  \includegraphics[scale=.6]{pictures/q_mean2}
\end{frame}
...
\begin{frame}
  ...
  \item removing unwanted latent factors as part of estimation
  \item extension to high-throughput methylation data
  \item variance estimate and goodness-of-fit 
  \end{enumerate}
\end{frame}
...
\begin{frame}
\begin{center}Extras\end{center}
\end{frame}
...
\begin{frame}
  ...
  \begin{enumerate}
    \item {\bf un-adjusted:} there are {\cbo 399} differentially expressed genes
  ...
  \alert{Some of the un-adjusted genes probably due to cell type.}
\end{frame}
