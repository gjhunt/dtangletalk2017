...
\begin{frame}
 ...
 \vspace*{-.5cm}
  ...
  \uncover<2->{\item {\bf Existing approach:} model and fit measured exprs. as $M\approx PR$ on \alert{linear} ({\color{NavyBlue} biologically plausible, not robust}) or \alert{$\log$} scale ({\color{NavyBlue} robust, not biologically plausible})}
  \uncover<3->{\hspace*{-1.75cm}\item {\bf dtangle's approach:}}
  \begin{enumerate}
    \uncover<3->{\item[(1)] model actual exprs. as $M\approx PR$ on \alert{linear} scale \uncover<4->{{\color{NavyBlue} (biologically plausible)}}}
    \uncover<5->{\item[(2)] model GEP tech. as linear on \alert{$\log$} scale \uncover<6->{{\color{NavyBlue} (robust)}} }
    \uncover<7->{\item[(3)] combine and simplify (1) and (2) with \alert{marker genes}, fit on \alert{$\log$} scale \uncover<8->{{\color{NavyBlue} (plausible, robust, closed form, fast)}}}
  \end{enumerate}
  \end{enumerate}
\end{frame}
