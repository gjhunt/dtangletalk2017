...
\begin{frame}
  ...
  \end{center}}
  \uncover<4->{{\large Measuring Gene Expression = Quantifying mRNA transcripts}\\}
...
\begin{frame}
  \begin{enumerate}
  \item For a given technology estimate $\slope$ globally using reference data sets where we know the amount and the expressions.
  \item We simply take median of regression slopes for a large set of spiked-in genes.
  \end{enumerate}
  \frametitle{We Estimate $\slope$ from Benchmark Data Sets}
  \begin{figure}
    \begin{overpic}[width=.65\textwidth,tics=10]{lin_approx}
      \put (-6,50) {$Y_{\cbr n}$}
      \put (11,10) {$\log_2\eta_{\cbr n}$}
    \end{overpic}
...
\begin{frame}
  \frametitle{We Choose Markers From Reference Pure Samples}
             {\bf Goal:} Determine which genes are highly expressed in some cell types but not others.\newline
             {\bf Method:} Some type of differential expression analysis among pure sample expressions.

             \alert{{\bf dtangle} isn't very sensitive to precisely how the differential expression analysis is done.}
\end{frame}
