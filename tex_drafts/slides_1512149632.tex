...
\begin{frame}
  \frametitle{Gene = Code Telling Cell How to Do Something}
             ...
             \uncover<4->{\alert{Measure gene exprs. to learn what cells are doing.}}
...
\begin{frame}
  ...
  \hspace{1cm}\begin{center}
    \begin{overprint}
      \onslide<1|handout:10>\hspace{-.75cm}\includegraphics[scale=.28]{pictures/mixture_cartoon}
      \onslide<2-|handout:10>\hspace*{-.77cm}\includegraphics[scale=.28]{pictures/mixture_cartoon_data}
    \end{overprint}
  \end{center}
...
\begin{frame}
  ...
  \begin{center}\begin{minipage}{\columnwidth}
    \centering
    \begin{overprint}
      \onslide<1|handout:10>\includegraphics[scale=.38]{pictures/confound}
      \onslide<2-|handout:10>\includegraphics[scale=.38]{pictures/confound2}
    \end{overprint}
  \end{minipage}
  \end{center}
    \end{column}
...
\begin{frame}
  ...
  \uncover<2->{\item mixture sample with unknown mixing proportions $\pa$, $\pb$}
  \uncover<3->{\item $\ycn,\yan,\ybn=\log_2$ expr. in {\crb mixture}, refs. {\crb A, B} of gene {\cbr \n}}.
  \uncover<4->{\item $\na$ and $\nb$ be marker genes of types $\crb A$ and $\crb B$.}
  \end{enumerate}

  \uncover<5->{The {\bf dtangled} estimator of $p_{\crb A}$ is}\vspace{.1cm}
    \begin{overprint}
    \onslide<5|handout:10>\[\widehat{\pa} = \logistic_2\left(\frac{\left(\yca-\ycb\right)-\left(\yaa-\ybb\right)}{\slopehat}\right)\]
    \onslide<6|handout:10>\[\widehat{\pa} = \logistic_2\left(\frac{\alert{\left(\yca-\ycb\right)}-\left(\yaa-\ybb\right)}{\slopehat}\right)\]
   \onslide<7|handout:10>\vspace{-.55cm}\[\widehat{\pa} = \logistic_2\overbrace{\left(\frac{\left(\yca-\ycb\right)-\left(\yaa-\ybb\right)}{\slopehat}\right)}^{\text{normalized rel. abundance of $\crb A$ over $\crb B$}}\]
  \end{overprint}
  \uncover<5->{and similarly for $\pb$ where $\logistic_2(x)=1/(1+2^{-x})$.}    
\end{frame}
...
\begin{frame}
 ...
 \vspace*{-.5cm}
  ...
  \begin{enumerate}
    ...
    \uncover<7->{\item[(3)] combine and simplify (1) and (2) with \alert{marker genes}, fit on \alert{$\log$} scale \uncover<8->{{\bf\color{blue} (robust, closed form, fast)}}}
  \end{enumerate}
  \end{enumerate}
\end{frame}
...
\begin{frame}
  ...
  We model \alert{measured expression} on \alert{actual expression} as linear:
      \begin{overprint}
      \onslide<1|handout:10>\begin{overpic}[scale=.23,tics=10]{pictures/ma_mod_plot_empty} \put (-6,50) {} \put (11,10) {} \end{overpic}
      \onslide<2-|handout:10>\begin{overpic}[scale=.23,tics=10]{pictures/ma_mod_plot} \put (-10,45) {\fbox{\alert{$Y_{\n}$}}} \put (50,-5) {\fbox{$\log_2$\alert{$\eta_{\n}$}}} \end{overpic}
      \end{overprint}
\uncover<3->{\hspace*{3.5cm}\begin{minipage}{\textwidth}\vspace*{-6cm}\[\boxed{
      \begin{aligned}
...
\begin{frame}
  ...
  \alert{(Defn) Marker gene:} acutally expressed in only one type. If\vspace{-.25cm}
      \begin{center}
        $\na$ = marker gene for type $\crb A$,
        $\nb$ = marker gene for type $\crb B$
      \end{center}\vspace{-.25cm}
      then $\eanb=0\text{ and }\ebna=0$.

 \uncover<2->{{\bf Combining this definition with our models:}}
  
  \uncover<2->{\fbox{\alert{Mixing Model:} $\ecn=\pa\ean+\pb\ebn$}}
  \begin{overprint}
    \onslide<3|handout:10>\[\ycn = \tn + \slope\log_2\left(\ecn\right)+\epscn\]
    \onslide<4-|handout:10>\[\ycn = \tn + \slope\log_2\underbrace{\left(\pa\ean+\pb\ebn\right)}_{\ecn}+\epscn\]
  \end{overprint}

  \uncover<5->{\fbox{\alert{Marker Gene Defn:} $\eanb=0\text{ and }\ebna=0$}}\vspace{-.25cm}
  \begin{overprint}
    \onslide<6|handout:10>\[\ycna = \tna + \slope\log_2\left(\pa\eana+\pb\eta_{\crb B \cbr \na} \right)+\epsilon_{\crb C \cbr g_A}\]
    \onslide<7|handout:10>\[\ycna = \tna + \slope\log_2\left(\pa\eana+\hcancel[red]{\color{gray}\pb\eta_{ B g_A}} \right)+\epsilon_{\crb C \cbr g_A}\]
    \onslide<8|handout:10>\[\ycna = \tna + \slope\log_2\left(\pa\eana\right)+\epsilon_{\crb C \cbr g_A}\]
  \end{overprint}
  \begin{overprint}
    \onslide<6|handout:10>\[\ycnb = \tnb + \slope\log_2\left(\pa\eta_{\crb A \cbr \nb}+\pb\ebnb\right)+\epsilon_{\crb C \cbr g_B}\]
    \onslide<7|handout:10>\[\ycnb = \tnb + \slope\log_2\left(\hcancel[red]{{\color{gray} \pa\eta_{Ag_B}}}+\pb\ebnb\right)+\epsilon_{\crb C \cbr g_B}\]
    \onslide<8|handout:10>\[\ycnb = \tnb + \slope\log_2\left(\pb\ebnb\right)+\epsilon_{\crb C \cbr g_B}\]
  \end{overprint}
