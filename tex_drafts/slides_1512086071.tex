...
\begin{frame}
  ...
  \uncover<1->{\item mixture sample with unknown mixing proportions $\pa$, $\pb$}
  \uncover<1->{\item $\ycn=\log_2\left(\text{measured expression of gene {\cbr $\n$} in {\crb mixture} sample}\right)$. Similarly define $\yan$ and $\ybn$.}
  \uncover<1->{\item $\na$ and $\nb$ be marker genes of types $\crb A$ and $\crb B$.}
  \end{enumerate}

\uncover<1->{The {\bf dtangled} estimator of $p_{\crb A}$ is
  \[\boxed{
\widehat{\pa} = \logistic_2\left(\frac{\left(\yca-\ycb\right)-\left(\yaa-\ybb\right)}{\slopehat}\right)
  ...
  \begin{enumerate}
  \uncover<1->{\item $\yca-\ycb =$ rel. abundance of $\crb A$ over $\crb B$ in the mixture}
  \uncover<1->{\item $\widehat{\pa} = \logistic(\text{normalized abundance of $\crb A$ over $\crb B$ in $\crb C$})$}
  \end{enumerate}
