...
\begin{frame}
 ...
 \vspace*{-.75cm}
  ...
  \hspace*{-1cm}\vspace{-1.25cm}\begin{enumerate}
...
\begin{frame}
  ...
  \uncover<3->{\item $\ycn=\log_2\left(\text{measured expression of gene {\cbr $\n$} in {\crb mixture} sample}\right)$. Similarly define $\yan$ and $\ybn$.}
  \uncover<4->{\item $\na$ and $\nb$ be marker genes of types $\crb A$ and $\crb B$.}
  \end{enumerate}

  \uncover<5->{The {\bf dtangled} estimator of $p_{\crb A}$ is
    \begin{overprint}
    \onslide<5>\[\boxed{\widehat{\pa} = \logistic_2\left(\frac{\left(\yca-\ycb\right)-\left(\yaa-\ybb\right)}{\slopehat}\right)}\]
    \onslide<6>\[\boxed{\widehat{\pa} = \logistic_2\left(\frac{\overbrace{\left(\yca-\ycb\right)}^{\text{rel. abundance of $\crb A$ over $\crb B$}}-\left(\yaa-\ybb\right)}{\slopehat}\right)}\]
   \onslide<7>\[\boxed{\widehat{\pa} = \logistic_2\overbrace{\left(\frac{\left(\yca-\ycb\right)-\left(\yaa-\ybb\right)}{\slopehat}\right)}^{\text{normalized rel. abundance of $\crb A$ over $\crb B$}}}\]
  }\]
  \end{overprint}
  and similarly for $\pb$ where $\logistic_2(x)=1/(1+2^{-x})$.}
  
  \begin{enumerate}
  \uncover<6->{\item $\yca-\ycb =$ rel. abundance of $\crb A$ over $\crb B$ in the mixture}
  \uncover<7->{\item $\widehat{\pa} = \logistic(\text{ in $\crb C$})$}
  \end{enumerate}
