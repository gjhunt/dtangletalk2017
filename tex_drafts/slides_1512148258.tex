...
\begin{frame}
  ...
  \hspace{1cm}\begin{center}
    ...
    \end{overprint}
  \end{center}
  \alert{\large Goal:} Computationally ``De-Mix'' the mixture = determine unknown mixing proportions
...
\begin{frame}
  ...
  \uncover<5->{The {\bf dtangled} estimator of $p_{\crb A}$ is}\vspace{.1cm}
   ...
   \onslide<7|handout:0>\vspace{-.55cm}\[\widehat{\pa} = \logistic_2\overbrace{\left(\frac{\left(\yca-\ycb\right)-\left(\yaa-\ybb\right)}{\slopehat}\right)}^{\text{normalized rel. abundance of $\crb A$ over $\crb B$}}\]
  \end{overprint}
