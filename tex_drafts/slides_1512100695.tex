...
\begin{frame}
  ...
  \end{center}
    ...
    \begin{column}{.4\textwidth}\vspace{-1.7cm}
      ...
      \uncover<5->{\fbox{\alert{\bf Solution:} Estimate the cell-type proportions.\\De-confound analysis with estimates.}}\vspace{1.8cm}
...
\begin{frame}
  ...
  \uncover<5->{The {\bf dtangled} estimator of $p_{\crb A}$ is}\vspace{.1cm}
    ...
    \onslide<6>\vspace{-1cm}\[\widehat{\pa} = \logistic_2\left(\frac{\overbrace{\left(\yca-\ycb\right)}^{\text{rel. abundance of $\crb A$ over $\crb B$}}-\left(\yaa-\ybb\right)}{\slopehat}\right)\]
   \onslide<7>\vspace{-.6cm}\[\widehat{\pa} = \logistic_2\overbrace{\left(\frac{\left(\yca-\ycb\right)-\left(\yaa-\ybb\right)}{\slopehat}\right)}^{\text{normalized rel. abundance of $\crb A$ over $\crb B$}}\]
  \end{overprint}
...
\begin{frame}
  ...
  \uncover<2->{\alert{(Defn) Marker gene:} acutally expressed in only one type. If
      \begin{center}
