...
\begin{frame}
  ...
  \hspace{1cm}\begin{center}
    \begin{overprint}
      \onslide<1|handout:1>\hspace{-.75cm}\includegraphics[scale=.28]{pictures/mixture_cartoon}
...
\begin{frame}
  ...
  \begin{center}\begin{minipage}{\columnwidth}
    \centering
    \begin{overprint}
      \onslide<1|handout:1>\includegraphics[scale=.38]{pictures/confound}
...
\begin{frame}
  ...
  \uncover<5->{The {\bf dtangled} estimator of $p_{\crb A}$ is}\vspace{.1cm}
    \begin{overprint}
    \onslide<5|handout:1>\[\widehat{\pa} = \logistic_2\left(\frac{\left(\yca-\ycb\right)-\left(\yaa-\ybb\right)}{\slopehat}\right)\]
    \onslide<6|handout:1>\[\widehat{\pa} = \logistic_2\left(\frac{\alert{\left(\yca-\ycb\right)}-\left(\yaa-\ybb\right)}{\slopehat}\right)\]
   \onslide<7|handout:1>\vspace{-.55cm}\[\widehat{\pa} = \logistic_2\overbrace{\left(\frac{\left(\yca-\ycb\right)-\left(\yaa-\ybb\right)}{\slopehat}\right)}^{\text{normalized rel. abundance of $\crb A$ over $\crb B$}}\]
  \end{overprint}
...
\begin{frame}
  ...
  We model \alert{measured expression} on \alert{actual expression} as linear:
      \begin{overprint}
      \onslide<1|handout:1>\begin{overpic}[scale=.23,tics=10]{pictures/ma_mod_plot_empty} \put (-6,50) {} \put (11,10) {} \end{overpic}
      \onslide<2-|handout:1>\begin{overpic}[scale=.23,tics=10]{pictures/ma_mod_plot} \put (-10,45) {\fbox{\alert{$Y_{\n}$}}} \put (50,-5) {\fbox{$\log_2$\alert{$\eta_{\n}$}}} \end{overpic}
      \end{overprint}
...
\begin{frame}
 ...
 \uncover<2->{{\bf Combining this definition with our models:}}
  ...
  \begin{overprint}
    \onslide<3|handout:1>\[\ycn = \tn + \slope\log_2\left(\ecn\right)+\epscn\]
    \onslide<4-|handout:1>\[\ycn = \tn + \slope\log_2\underbrace{\left(\pa\ean+\pb\ebn\right)}_{\ecn}+\epscn\]
  \end{overprint}

  \uncover<5->{\fbox{\alert{Marker Gene Defn:} $\eanb=0\text{ and }\ebna=0$}}\vspace{-.25cm}
  \begin{overprint}
    \onslide<6|handout:1>\[\ycna = \tna + \slope\log_2\left(\pa\eana+\pb\eta_{\crb B \cbr \na} \right)+\epsilon_{\crb C \cbr g_A}\]
    \onslide<7|handout:1>\[\ycna = \tna + \slope\log_2\left(\pa\eana+\hcancel[red]{\color{gray}\pb\eta_{ B g_A}} \right)+\epsilon_{\crb C \cbr g_A}\]
    \onslide<8|handout:1>\[\ycna = \tna + \slope\log_2\left(\pa\eana\right)+\epsilon_{\crb C \cbr g_A}\]
  \end{overprint}
  \begin{overprint}
    \onslide<6|handout:1>\[\ycnb = \tnb + \slope\log_2\left(\pa\eta_{\crb A \cbr \nb}+\pb\ebnb\right)+\epsilon_{\crb C \cbr g_B}\]
    \onslide<7|handout:1>\[\ycnb = \tnb + \slope\log_2\left(\hcancel[red]{{\color{gray} \pa\eta_{Ag_B}}}+\pb\ebnb\right)+\epsilon_{\crb C \cbr g_B}\]
    \onslide<8|handout:1>\[\ycnb = \tnb + \slope\log_2\left(\pb\ebnb\right)+\epsilon_{\crb C \cbr g_B}\]
  \end{overprint}
