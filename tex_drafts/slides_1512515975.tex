...
\begin{frame}
  ...
  \begin{columns}
    ...
    \begin{column}{.75\textwidth}
      ...
      \item can lead to worse conditions like Lyme encephalopathy, insomina, or depression
      \end{enumerate}
...
\begin{frame}
  \frametitle{Study WBCs to Understand Immune Response to Lyme}
             {\bf Bouquet et al:} collect gene expression profiles (GEPs) of white blood cells (WBCs) of 
             \begin{enumerate}
             ...
             \vspace{.5cm}\uncover<2->{\alert{WBCs encompass many types:} B,T,NK,monocytes,...\\\vspace{.25cm}
               {\bf Understanding these sub-types would be helpful:}
               \begin{enumerate}
               \item tracking subtype composition changes over disease course
               \item adjusting GEP comparisons across groups
               \end{enumerate}
             }

             \uncover<3->{{\bf \alert{Problem:}} estimate the cell-type proportions of the samples using the gene expression data.}
\end{frame}
