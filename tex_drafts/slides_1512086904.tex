...
\begin{frame}
  ...
  \uncover<5->{The {\bf dtangled} estimator of $p_{\crb A}$ is}
    ...
    \onslide<6>\vspace{-1cm}\[\boxed{\widehat{\pa} = \logistic_2\left(\frac{\overbrace{\left(\yca-\ycb\right)}^{\text{rel. abundance of $\crb A$ over $\crb B$}}-\left(\yaa-\ybb\right)}{\slopehat}\right)}\]
   \onslide<7>\vspace{-1cm}\[\boxed{\widehat{\pa} = \logistic_2\overbrace{\left(\frac{\left(\yca-\ycb\right)-\left(\yaa-\ybb\right)}{\slopehat}\right)}^{\text{normalized rel. abundance of $\crb A$ over $\crb B$}}}\]
  \end{overprint}
