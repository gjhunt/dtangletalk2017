...
M = PR + E
  \]
  where $M$ and $R$ are known and we try to predict $P$, 
...
\begin{frame}
  ...
  \begin{block}{Problem}
    ...
    \]
    and solve for $P$.
    {\bf Constraint:} $P$ must be a row-wise probability matrix. 
  \end{block}\vspace{-.25cm}
  {\bf Solutions:}
  \begin{enumerate}
  \item {\bf{\color{blue}Regression}: regress $M$ on $R$ so that elements of $M$ are regression coefficients. Somehow enforce constraints.}\\ (\citeauthor{Abbas2009,Gong2011,Lu2003,Wang2006,Qiao2012,Altboum2014,Newman2015})
...
\begin{frame}
  ...
  \begin{enumerate}
    ...
    \uncover<5->{\item $\yaa,\ybb=\log_2\left(\text{expresssion of markers $\na,\nb$ in the {\crb refs.}}\right)$}
  \end{enumerate}

  \uncover<6->{The {\bf dtangled} estimator of $\pa$ is}\vspace{.1cm}
    \begin{overprint}
    \onslide<6|handout:0>\[\widehat{\pa} = \logistic_2\left(\frac{\left(\yca-\ycb\right)-\left(\yaa-\ybb\right)}{\slopehat}\right)\]
    \onslide<7|handout:0>\[\widehat{\pa} = \logistic_2\left(\frac{\alert{\left(\yca-\ycb\right)}-\left(\yaa-\ybb\right)}{\slopehat}\right)\]
   \onslide<8|handout:0>\vspace{-.57cm}\[\widehat{\pa} = \logistic_2\overbrace{\left(\frac{\alert{\left(\yca-\ycb\right)}-\left(\yaa-\ybb\right)}{\slopehat}\right)}^{\text{normalized rel. abundance of $\crb A$ over $\crb B$}}\]
  \end{overprint}
  \uncover<6->{and similarly for $\pb$ where $\logistic_2(x)=1/(1+2^{-x})$.}    
\end{frame}
...
\begin{frame}
 ...
 \vspace*{-.5cm}
  ...
  \begin{enumerate}
    \uncover<3->{\item[(1)] model actual exprs. as $M=PR$ on \alert{linear} scale \uncover<4->{{\bf\color{blue} (plausible)}}}
...
\begin{frame}
 ...
 \uncover<2->{{\bf Combining this definition with our models:}}
  ...
  \begin{overprint}
    \onslide<6|handout:0>\[\ycna = \tna + \slope\log_2\left(\pa\eana+\pb {\Large R}_{\crb B \cbr \na} \right)+\epsilon_{\cbr a}\]
    \onslide<7|handout:0>\[\ycna = \tna + \slope\log_2\left(\pa\eana+\hcancel[red]{\color{gray}\pb\eta_{ B g_A}} \right)+\epsilon_{\cbr a}\]
    \onslide<8|handout:0>\[\ycna = \tna + \slope\log_2\left(\pa\eana\right)+\epsilon_{\cbr a}\]
  \end{overprint}
  \begin{overprint}
    \onslide<6|handout:0>\[\ycnb = \tnb + \slope\log_2\left(\pa \text{\Large $\widetilde{R}$}_{\crb A \cbr \nb}+\pb\ebnb\right)+\epsilon_{\cbr b}\]
    \onslide<7|handout:0>\[\ycnb = \tnb + \slope\log_2\left(\hcancel[red]{{\color{gray} \pa\eta_{Ag_B}}}+\pb\ebnb\right)+\epsilon_{\cbr b}\]
