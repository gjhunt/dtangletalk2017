...
\begin{frame}
  ...
  \uncover<5->{{\large \vspace{-.5cm}{\bf How can we measure gene expression?}\\Gene Expressed $\rightarrow$ \alert{mRNA transcript} created}\\}
  ...
  \uncover<8->{\begin{center}
    \vspace{-1cm}\hspace*{-.75cm}\includegraphics[scale=.26]{pictures/measure_expr}
  \end{center}}
...
\begin{frame}
  ...
  \uncover<5->{The {\bf dtangled} estimator of $p_{\crb A}$ is}\vspace{.1cm}
    \begin{overprint}
    \onslide<5>\[\widehat{\pa} = \logistic_2\left(\frac{\left(\yca-\ycb\right)-\left(\yaa-\ybb\right)}{\slopehat}\right)\]
    \onslide<6>\[\widehat{\pa} = \logistic_2\left(\frac{\alert{\left(\yca-\ycb\right)}-\left(\yaa-\ybb\right)}{\slopehat}\right)\]
   \onslide<7>\vspace{-.55cm}\[\widehat{\pa} = \logistic_2\overbrace{\left(\frac{\left(\yca-\ycb\right)-\left(\yaa-\ybb\right)}{\slopehat}\right)}^{\text{normalized rel. abundance of $\crb A$ over $\crb B$}}\]
  \end{overprint}
...
\begin{frame}
  ...
  We model \alert{measured expression} on \alert{actual expression} as linear:
      \begin{overprint}
      \onslide<1>\begin{overpic}[scale=.23,tics=10]{pictures/ma_mod_plot_empty} \put (-6,50) {} \put (11,10) {} \end{overpic}
      \onslide<2->\begin{overpic}[scale=.23,tics=10]{pictures/ma_mod_plot} \put (-10,45) {\fbox{\alert{$Y_{\n}$}}} \put (50,-5) {\fbox{$\log_2$\alert{$\eta_{\n}$}}} \end{overpic}
      \end{overprint}
