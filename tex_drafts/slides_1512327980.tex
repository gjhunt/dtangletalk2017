...
\begin{frame}
 ...
 \vspace*{-.5cm}
  ...
  \uncover<2->{\item {\bf Existing approach:} model and fit measured exprs. as $M=PR+E$ on \alert{linear} ({\color{blue} biologically plausible, not robust}) or \alert{$\log$} scale ({\color{blue} robust, not biologically plausible})}
  \uncover<3->{\hspace*{-1.75cm}\item {\bf dtangle's approach:}}
  \begin{enumerate}
    \uncover<3->{\item[(1)] model actual exprs. as $M=PR$ on \alert{linear} scale \uncover<4->{{\color{blue} (biologically plausible)}}}
