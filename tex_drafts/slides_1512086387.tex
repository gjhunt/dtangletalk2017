...
\uncover<5->{The {\bf dtangled} estimator of $p_{\crb A}$ is
  \[\boxed{
    ...
    \widehat{\pa} = \logistic_2\left(\frac{\overbrace{\left(\yca-\ycb\right)}^{\text{rel. abundance of $\crb A$ over $\crb B$}}-\left(\yaa-\ybb\right)}{\slopehat}\right)
   %\widehat{\pa} = \logistic_2\overbrace{\left(\frac{\left(\yca-\ycb\right)-\left(\yaa-\ybb\right)}{\slopehat}\right)}^{\text{normalized rel. abundance of $\crb A$ over $\crb B$}}
}\]
