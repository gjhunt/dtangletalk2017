...
\begin{frame}
  ...
  \hspace*{1cm}$\ecna = \text{actual expression  of gene {\cbr \na} in {\crb mixture sample}}$\\(and similarly for $\eana$ and $\ebna$ etc.),\vspace{-.25cm}
  \[
  \ecna=\pa\eana+\pb\ebna.
  \]
    {\bf\large (Step 2)} model GEP tech. as linear on \alert{$\log$-$\log$} scale:\\
    \[
    \begin{aligned}
      \ycna &= \tna + \slope\log_2\ecna+\epscna\\
      \yana &= \tna + \slope\log_2\eana+\epsana\\
    \end{aligned}
    \]
