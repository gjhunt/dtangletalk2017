...
\begin{frame}
  ...
  We model \alert{measured expression} on \alert{actual expression} as linear:
      \begin{overprint}
      \onslide<1>\begin{overpic}[scale=.24,tics=10]{pictures/ma_mod_plot_empty} \put (-6,50) {} \put (11,10) {} \end{overpic}
      \onslide<2>\begin{overpic}[scale=.24,tics=10]{pictures/ma_mod_plot} \put (-10,45) {\fbox{\alert{$Y_{\n}$}}} \put (50,-5) {\fbox{$\log_2$\alert{$\eta_{\n}$}}} \end{overpic}
      \end{overprint}
\hspace*{4.5cm}\begin{minipage}{\textwidth}\vspace*{-7cm}\[
      \begin{aligned}
