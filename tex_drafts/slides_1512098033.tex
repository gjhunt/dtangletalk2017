...
\begin{frame}
  ...
  \uncover<2->{{\bf dtangle:} model mixing of \alert{actual} expressions on the \alert{linear} scale:
  \begin{center}
...
\begin{frame}
  ...
  \uncover<2->{\fbox{The linear model: $\alert{Y_{\n}} = \theta_{\n}+\gamma\log_2\left(\alert{\eta_{\n}}\right)+\epsilon$}}
    \begin{center}
      ...
      \onslide<2>\begin{overpic}[width=.75\textwidth,tics=10]{pictures/ma_mod_plot} \put (-10,45) {\fbox{\alert{$Y_{\n}$}}} \put (50,-5) {\fbox{$\log_2$\alert{$\eta_{\n}$}}} \end{overpic}
...
\begin{frame}
  \frametitle{dtangle's Models GEP Technology}
  ...
  \uncover<2->{{\large \alert{(Defn) Marker gene:} acutally expressed in only one type}  
