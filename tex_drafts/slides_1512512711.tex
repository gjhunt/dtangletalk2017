...
\begin{frame}
  \frametitle{Study WBCs to Understand Immune Response to Lyme}
             ...
             \uncover<3->{\bf \alert{Problem: estimate the cell-type proportions of the samples.}}
\end{frame}

\begin{frame}
  \frametitle{Genomics Review}
             {\Large Gene ``expressed'' = Gene used by cell}\\\vspace{.1cm}
             {\Large \alert{We measure gene exprs. to learn what cells are doing.}}
             
  \uncover<2->{{\large {\bf Measure expression using mRNA:} Gene Expressed $\rightarrow$ \alert{mRNA transcript} created}\\
  \begin{center}
