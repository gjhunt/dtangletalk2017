...
\begin{frame}
  ...
  \begin{enumerate}
  \item Two cell types: $\crb A$ and $\crb B$
  \item mixture sample with unknown mixing proportions $\pa$, $\pb$
  \item $\ycn=\log_2\left(\text{measured expression of gene {\cbr $\n$} in {\crb mixture} sample}\right)$. Similarly define $\yan$ and $\ybn$.
...
\widehat{\pa} = \logistic_2\left(\frac{\left(\yca-\ycb\right)-\left(\yaa-\ybb\right)}{\slopehat}\right)
  ...
  \begin{enumerate}
  \item $\yca-\ycb$ is the relative abundance of type $\crb A$ over $\crb B$ in the mixture
