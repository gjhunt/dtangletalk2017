...
\newcommand{\yan}{\text{\Large $R$\hspace*{-.05cm}}_{{\crb A}}}
\newcommand{\ybn}{\text{\Large $R$\hspace*{-.05cm}}_{{\crb B}}}
...
\newcommand{\ean}{\text{\Large $\tilde{R}$}_{{\crb A}{\cbr g}}}
\newcommand{\ebn}{\text{\Large $\tilde{R}$}_{{\crb B}{\cbr g}}}
\newcommand{\ecn}{\text{\Large $\tilde{M}$}_{{\cbr g}}}

\newcommand{\eana}{\text{\Large $\tilde{R}$}_{{\crb A}{\cbr g_A}}}
\newcommand{\ebna}{\text{\Large $\tilde{R}$}_{{\crb B}{\cbr g_A}}}
\newcommand{\ecna}{\text{\Large $\tilde{M}$}_{{\cbr g_A}}}
\newcommand{\eanb}{\text{\Large $\tilde{R}$}_{{\crb A}{\cbr g_B}}}
\newcommand{\ebnb}{\text{\Large $\tilde{R}$}_{{\crb B}{\cbr g_B}}}
\newcommand{\ecnb}{\text{\Large $\tilde{M}$}_{{\cbr g_B}}}
...
\begin{frame}
  ...
  \begin{enumerate}
    ...
    \uncover<3->{\item mixture sample $M$ with unknown mixing proportions $\pa$, $\pb$}
    \uncover<4->{\item $\yca,\ycb=\log_2\left(\text{expresssion of markers $\na,\nb$ in the {\crb mixture}}\right)$}
    \uncover<5->{\item $\yaa,\ybb=\log_2\left(\text{expresssion of markers $\na,\nb$ in the {\crb references}}\right)$}
  \end{enumerate}

  \uncover<6->{The {\bf dtangled} estimator of $\pa$ is}\vspace{.1cm}
    \begin{overprint}
    \onslide<7|handout:0>\[\widehat{\pa} = \logistic_2\left(\frac{\left(\yca-\ycb\right)-\left(\yaa-\ybb\right)}{\slopehat}\right)\]
    \onslide<8|handout:0>\[\widehat{\pa} = \logistic_2\left(\frac{\alert{\left(\yca-\ycb\right)}-\left(\yaa-\ybb\right)}{\slopehat}\right)\]
   \onslide<9|handout:0>\vspace{-.57cm}\[\widehat{\pa} = \logistic_2\overbrace{\left(\frac{\alert{\left(\yca-\ycb\right)}-\left(\yaa-\ybb\right)}{\slopehat}\right)}^{\text{normalized rel. abundance of $\crb A$ over $\crb B$}}\]
  \end{overprint}
...
\begin{frame}
  ...
  We model \alert{measured expression} on \alert{actual expression} as linear:
      ...
      \onslide<2-|handout:0>\begin{overpic}[scale=.23,tics=10]{pictures/ma_mod_plot} \put (-10,45) {\fbox{\alert{$Y_{\n}$}}} \put (50,-5) {\fbox{$\log_2$\alert{$\eta_{\n}$}}} \end{overpic}
      \end{overprint}
...
\begin{frame}
 ...
 \uncover<2->{{\bf Combining this definition with our models:}}
  ...
  \begin{overprint}
    \onslide<6|handout:0>\[\ycna = \tna + \slope\log_2\left(\pa\eana+\pb\eta_{\crb B \cbr \na} \right)+\epsilon_{\crb C \cbr g_A}\]
    \onslide<7|handout:0>\[\ycna = \tna + \slope\log_2\left(\pa\eana+\hcancel[red]{\color{gray}\pb\eta_{ B g_A}} \right)+\epsilon_{\crb C \cbr g_A}\]
    \onslide<8|handout:0>\[\ycna = \tna + \slope\log_2\left(\pa\eana\right)+\epsilon_{\crb C \cbr g_A}\]
  \end{overprint}
  \begin{overprint}
    \onslide<6|handout:0>\[\ycnb = \tnb + \slope\log_2\left(\pa\eta_{\crb A \cbr \nb}+\pb\ebnb\right)+\epsilon_{\crb C \cbr g_B}\]
    \onslide<7|handout:0>\[\ycnb = \tnb + \slope\log_2\left(\hcancel[red]{{\color{gray} \pa\eta_{Ag_B}}}+\pb\ebnb\right)+\epsilon_{\crb C \cbr g_B}\]
    \onslide<8|handout:0>\[\ycnb = \tnb + \slope\log_2\left(\pb\ebnb\right)+\epsilon_{\crb C \cbr g_B}\]
  \end{overprint}
