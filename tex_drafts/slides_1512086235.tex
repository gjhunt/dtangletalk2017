...
\uncover<5->{The {\bf dtangled} estimator of $p_{\crb A}$ is
  \[\boxed{
\widehat{\pa} = \logistic_2\left(\overbrace{\frac{\left(\yca-\ycb\right)-\left(\yaa-\ybb\right)}{\slopehat}}^{abc}\right)
}\]
  and similarly for $\pb$ where $\logistic_2(x)=1/(1+2^{-x})$.}
  
  \begin{enumerate}
  \uncover<6->{\item $\yca-\ycb =$ rel. abundance of $\crb A$ over $\crb B$ in the mixture}
  \uncover<7->{\item $\widehat{\pa} = \logistic(\text{normalized abundance of $\crb A$ over $\crb B$ in $\crb C$})$}
  \end{enumerate}
