...
\begin{frame}
 ...
 \vspace*{-.5cm}
  ...
  \uncover<2->{\item {\bf Existing approach:} model and fit measured exprs. as $M\approx PR$ on \alert{linear} ({\color{blue} biologically plausible, not robust}) or \alert{$\log$} scale ({\color{blue} robust, not biologically plausible})}
  \uncover<3->{\hspace*{-1.75cm}\item {\bf dtangle's approach:}}
  \begin{enumerate}
    \uncover<3->{\item[(1)] model actual exprs. as $M\approx PR$ on \alert{linear} scale \uncover<4->{{\color{blue} (biologically plausible)}}}
    \uncover<5->{\item[(2)] model GEP tech. as linear on \alert{$\log$} scale \uncover<6->{{\color{blue} (robust)}} }
    \uncover<7->{\item[(3)] combine and simplify (1) and (2) with \alert{marker genes}, fit on \alert{$\log$} scale \uncover<8->{{\color{blue} (plausible, robust, closed form, fast)}}}
  \end{enumerate}
  \end{enumerate}
\end{frame}

\begin{frame}
  \frametitle{(Step 1) dtangle Models Actual Expression Mixing}
...
\begin{frame}
  ...
  \alert{\bf (Defn) Marker gene:} acutally expressed in only one type.\\
  \begin{center}
...
\begin{frame}
  \frametitle{dtangle's approach}  
             ...
             \{\bf\large (Step 2)} model of GEP technology:       \onslide<2-3>\[\begin{aligned}\ycna &= \tna + \slope\log_2\left(\ecna\right)+\epscna\\\yana &= \tna + \slope\log_2\left(\eana\right)+\epsana\\\end{aligned}\]
               {\bf\large (Step 3)} define marker genes: $\eanb=0\text{ and }\ebna=0$

               
      %\[\begin{aligned}\ycna &= \tna + \slope\log_2\left(\pa\eana+\pb\ebna\right)+\epscna\\\yana &= \tna + \slope\log_2\left(\eana\right)+\epsana\\\end{aligned}\]
      %\[\begin{aligned}\ycna &= \tna + \slope\log_2\left(\pa\eana\right)+\epscna\\\yana &= \tna + \slope\log_2\left(\eana\right)+\epsana\\\end{aligned}\]
      %\[\frac{\ycna-\yana}{\slope} = \log_2\left(\pa\right)+\epsilon\]
      %\[\exp_2\left(\frac{\ycna-\yana}{\slope}\right) \approx \pa\]
...
\begin{frame}
  ...
  We can show that,
    \begin{overprint}
      \onslide<1>\begin{center}$\pa\approx\exp_2\left(\frac{\ycna-\yana}{\slope}\right)$ and $\pb\approx\exp_2\left(\frac{\ycnb-\ybnb}{\slope}\right)$\end{center}
      \onslide<2->\begin{center}$\pa\approx\exp_2\left(\frac{\ycna-\yana}{\slopehat}\right)$ and $\pb\approx\exp_2\left(\frac{\ycnb-\ybnb}{\slopehat}\right)$\end{center}
    \end{overprint}
  \uncover<3->{they are not nice since
  \begin{enumerate}
  \item they are not bounded above by 1
  \item they do not sum to 1. 
  \end{enumerate}}
  \uncover<4->{
    We can fix this by re-normalizing each by their sum, and estimate $\pa$ by \vspace{-.5cm}
    \[
    \begin{aligned}
    ...
    \uncover<5->{&= \boxed{\logistic_2\left(\frac{\left(\yca-\yaa\right)}{\slopehat}-\frac{\left(\ycb-\ybb\right)}{\slopehat}\right)}}
    \end{aligned}
    \]
  }
\end{frame}
...
\]\vspace{-.5cm}there is we have a direct generalization
...
\begin{frame}
  ...
  Bouquet et al. (2016) we compare the control group to Lyme patients:
  \begin{enumerate}
...
\begin{frame}
  \frametitle{dtangle's approach}  
  {\bf\large (Step 1)} model mixing of \alert{actual} expressions on the \alert{linear} scale:\vspace{-.5cm}
  \[
  \begin{aligned}
  \ycna &= \log_2\left(\text{measured expression  of gene {\cbr \na} in {\crb mixture sample}}\right)\\
  \ecna &= \text{actual expression  of gene {\cbr \na} in {\crb mixture sample}}
  \end{aligned}
  \]

  \vspace{-.75cm}(and similarly for $\eana$ and $\ebna$ etc.),\\
